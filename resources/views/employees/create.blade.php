<x-layouts::app :title="__('Add Employee')">
    <div class="p-6 max-w-3xl mx-auto space-y-6">
        <div class="rounded-3xl bg-gradient-to-r from-indigo-600 via-slate-700 to-zinc-800 p-8 text-white shadow-xl shadow-slate-200/60 dark:shadow-black/20">
            ...
            <h1 class="mt-3 text-3xl font-black">Ajouter un employé</h1>
            <p class="mt-3 max-w-2xl text-white/75">
                Créez une fiche employé pour votre agence.
            </p>
        </div>
            ...
            <form action="{{ route('employees.store') }}" method="POST" class="space-y-6">
                ...
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-zinc-700 dark:bg-zinc-900/60">
                    <label class="flex items-center gap-3 font-semibold text-slate-800 dark:text-slate-200">
                        <input type="checkbox" name="create_staff_account" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" @checked(old('create_staff_account', '1') === '1')>
                        Créer un compte de connexion pour cet employé
                    </label>

                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        <div>
                            <label class="mb-1 block font-semibold text-slate-700 dark:text-slate-300" for="password">Mot de passe temporaire</label>
