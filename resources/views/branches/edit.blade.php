<x-layouts::app :title="__('Edit Branch')">
    <div class="p-6 max-w-3xl mx-auto space-y-6">
        <div class="rounded-3xl bg-gradient-to-r from-indigo-600 via-slate-700 to-zinc-800 p-8 text-white shadow-xl shadow-slate-200/60 dark:shadow-black/20">
            ...
            <h1 class="mt-3 text-3xl font-black">Modifier la branche</h1>
            <p class="mt-3 max-w-2xl text-white/75">
                Modifiez les informations du site et son statut.
            </p>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/50 dark:border-zinc-700 dark:bg-zinc-800">
            <form action="{{ route('branches.update', $branch) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $branch->name) }}" required class="input-modern">
                </div>

                <div>
                    <label class="mb-1 block font-semibold text-slate-700 dark:text-slate-300" for="code">Code</label>
                    <input type="text" name="code" id="code" value="{{ old('code', $branch->code) }}" class="input-modern">
                </div>

                <div>
                    <label class="mb-1 block font-semibold text-slate-700 dark:text-slate-300" for="address">Adresse</label>
                    <input type="text" name="address" id="address" value="{{ old('address', $branch->address) }}" class="input-modern">
                </div>

                <div>
                    <label class="mb-1 block font-semibold text-slate-700 dark:text-slate-300" for="phone_number">Téléphone</label>
                    <input type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $branch->phone_number) }}" class="input-modern">
                </div>

                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block font-semibold text-slate-700 dark:text-slate-300" for="opening_time">Ouverture</label>
                        <input type="time" name="opening_time" id="opening_time" value="{{ old('opening_time', substr($branch->opening_time, 0, 5)) }}" required class="input-modern">
                    </div>
                    <div>
                        <label class="mb-1 block font-semibold text-slate-700 dark:text-slate-300" for="closing_time">Fermeture</label>
                        <input type="time" name="closing_time" id="closing_time" value="{{ old('closing_time', substr($branch->closing_time, 0, 5)) }}" required class="input-modern">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="simultaneous_capacity" id="simultaneous_capacity" value="{{ old('simultaneous_capacity', $branch->simultaneous_capacity) }}" min="1" max="100" required class="input-modern">
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Nombre maximal de véhicules acceptés sur le même créneau.</p>
                </div>
                ...
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-zinc-700 dark:bg-zinc-900/60">
                    ...
                    <label class="flex items-center gap-3 font-semibold text-slate-800 dark:text-slate-200">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" @checked((string) old('is_active', $branch->is_active ? '1' : '0') === '1')>
                        Branche active
                    </label>
                </div>
